...
/**
 * Mobile chrome — fixed-position top header + bottom-tab nav + drawers.
 * Renders only on mobile via Tailwind `md:hidden`. Does NOT wrap children;
 * pages flow normally underneath, with `body` getting padding via global CSS.
 */
export function MobileChrome() {
  ...
  // Hide entirely inside the admin
  ...
  return (
    <div className="md:hidden">
      {/* Floating menu button — top-right, no header bar */}
      <button
        onClick={() => {
          buzz();
          setMenuOpen(true);
        }}
        className={`fixed top-4 right-4 z-[100] w-10 h-10 rounded-full flex flex-col items-center justify-center gap-[5px] transition-all duration-300 ${
          scrolled
            ? "bg-deep/85 backdrop-blur-md border border-teal/15"
            : "bg-deep/35 backdrop-blur-sm"
        }`}
        aria-label="Open menu"
      >
        <span className="block w-4 h-0.5 bg-warm-white" />
        <span className="block w-4 h-0.5 bg-warm-white" />
        <span className="block w-4 h-0.5 bg-warm-white" />
      </button>
          ...
          <Drawer.Content className="fixed top-0 right-0 bottom-0 z-[201] w-[85vw] max-w-sm bg-deep-light border-l border-teal/20 flex flex-col">
            ...
            <div className="px-6 py-6 border-t border-teal/10 space-y-3">
              <a
                ...
                onClick={() => events.whatsappClick("mobile-chrome-side-menu")}

...
All browser automation in this skill uses the `agent-browser` CLI. The daemon starts automatically on first use and maintains persistent browser state (cookies, localStorage, sessions) across commands.

| Action | Command |
|--------|---------|
| Navigate to URL | `agent-browser open <url>` |
| Take screenshot | `agent-browser screenshot` |
| Annotated screenshot (with element refs) | `agent-browser screenshot --annotate` |
...
| Scroll by pixels | `agent-browser scroll <pixels>` |
| Execute JavaScript | `agent-browser eval "JS_CODE"` |
| Execute JS from file | `agent-browser eval "$(cat script.js)"` |
| Get element text | `agent-browser get-text @eN` |
| Get element attribute | `agent-browser get-attribute @eN "attr"` |
| Close browser | `agent-browser close` |

**Element refs** (`@e1`, `@e2`, etc.) are obtained from `agent-browser snapshot -i`. They are compact identifiers that reduce context usage vs full CSS selectors. You can also use CSS selectors directly in `agent-browser eval` calls.

**For complex JS extraction scripts**: Save the script to a file (e.g., `scripts/extract-styles.js`) and run with `agent-browser eval "$(cat scripts/extract-styles.js)"`. This avoids shell quoting issues with inline JS.

## Pre-Flight

1. **Browser automation is required via `agent-browser`.** Verify agent-browser is installed by running `agent-browser --version`. If it's not installed, install it with `npm install -g agent-browser && agent-browser install`. The daemon starts automatically on first use and maintains persistent browser state across commands. This skill cannot work without agent-browser.
2. Parse `the target URL provided by the user` as one or more URLs. Normalize and validate each URL; if any are invalid, ask the user to correct them before proceeding. For each valid URL, verify it is accessible by running `agent-browser open <url>` and confirming the page loads.
...
The spec file is not optional. It is not a nice-to-have. If you dispatch a builder without first writing a spec file, you are shipping incomplete instructions based on whatever you can remember from a agent-browser session, and the builder will guess to fill gaps.
...
### Screenshots
- Take **full-page screenshots** at desktop (1440px) and mobile (390px) viewports using `agent-browser screenshot --full-page`
- Use `agent-browser screenshot --annotate` for annotated screenshots with element refs when you need to identify interactive elements
...
**Fonts** — Inspect `<link>` tags for Google Fonts or self-hosted fonts via `agent-browser eval "JSON.stringify([...document.querySelectorAll('link[rel=stylesheet]')].map(l => l.href))"`. Check computed `font-family` on key elements (headings, body, code, labels). Document every family, weight, and style actually used. Configure them in `src/app/layout.tsx` using `next/font/google` or `next/font/local`.
...
**Scroll sweep:** Scroll the page slowly from top to bottom using `agent-browser scroll <pixels>` in increments. At each section, pause and observe with `agent-browser screenshot`:
...
**Click sweep:** Use `agent-browser snapshot -i` to get interactive element refs, then click every element that looks interactive with `agent-browser click @eN`:
...
**Hover sweep:** Hover over every element that might have hover states with `agent-browser hover @eN`:
- Buttons, cards, links, images, nav items
- Record what changes: color, scale, shadow, underline, opacity

**Responsive sweep:** Test at 3 viewport widths using `agent-browser eval "window.resizeTo(WIDTH, 900)"` or equivalent viewport resize:
- Desktop: 1440px
- Tablet: 768px
- Mobile: 390px
- At each width, take a screenshot with `agent-browser screenshot` and note which sections change layout (column → stack, sidebar disappears, etc.) and at approximately which breakpoint the change occurs.
...
```javascript
// Save this as scripts/discover-assets.js, then run via: agent-browser eval "$(cat scripts/discover-assets.js)"
// Or run inline with: agent-browser eval "<the JS below>"
JSON.stringify({
...
1. **Screenshot** the section in isolation — scroll to it with `agent-browser scroll <pixels>`, then `agent-browser screenshot`. Save to `docs/design-references/`.

2. **Extract CSS** for every element in the section. Save the extraction script below to a file and run it via `agent-browser eval "$(cat scripts/extract-styles.js)"` — don't hand-measure individual properties. Run it once per component container and capture the full output:

```javascript
// Per-component extraction — save as scripts/extract-styles.js
// Run via: agent-browser eval "$(cat scripts/extract-styles.js)"
// Or for a specific selector: agent-browser eval "(function(selector) { ... })('YOUR_SELECTOR')"
...
agent-browser eval "$(cat scripts/extract-styles.js)"  # captures State A

# Then trigger the state change via agent-browser CLI:
agent-browser scroll 500          # scroll to trigger
# or: agent-browser click @eN    # click to trigger
# or: agent-browser hover @eN    # hover to trigger

# State B: re-run the extraction script on the same element
agent-browser eval "$(cat scripts/extract-styles.js)"  # captures State B
...
4. **Extract real content** — all text, alt attributes, aria labels, placeholder text. Use `agent-browser eval "document.querySelector('SELECTOR').textContent"` or `agent-browser get-text @eN` for each text node. For tabbed/stateful content, **click each tab with `agent-browser click @eN` and extract content per state**.
